...
exports.findAll =async (req, res) => {
  let invitation=await Invitation.findAll({where : {userId: req.params.id}, include: [{ model: Contacts, as: 'contact' },{ model: Events, as: 'event' }] })
  //console.log(invitation)
  let Accepted=await Invitation.findAll({where : {userId: req.params.id,status:'Accepted'}, include: [{ model: Contacts, as: 'contact' },{ model: Events, as: 'event' }] })
  let Rejected=await Invitation.findAll({where : {userId: req.params.id,status:'Rejected'}, include: [{ model: Contacts, as: 'contact' },{ model: Events, as: 'event' }] })
  let Attended=await Invitation.findAll({where : {userId: req.params.id,status:'Accepted',attended:{[Op.gt]:0}}, include: [{ model: Contacts, as: 'contact' },{ model: Events, as: 'event' }] })
  let NotReplied=await Invitation.findAll({where : {userId: req.params.id,status:{[Op.is]:null}}, include: [{ model: Contacts, as: 'contact' },{ model: Events, as: 'event' }] })
...
invitation.event.message=user_event.message;
  invitation.event.reminderMessage=user_event.reminderMessage;
  invitation.event.title=user_event.title;
  invitation.event.description=user_event.description;
...
//"totalCount":88,"declinedCount":4,"acceptCount":81,"nullCount":3,"totalGuestAllowed":105/
			  	 connection.query('SELECT totalguest  FROM hayyacom.`User_Events` A  where A.WEventId="'+eventid+'" and A.UserId = '+userid, (err, totalguest_rows) => {
		...
		//const accepted= await connection.query('SELECT * FROM hayyacom.`W-Events` WHERE id="'+eventid+'"  ')
			//guestattended += parseInt(invitations[0].attendedresult||"0"); 
        ...
        res.status(200).send({"totalGuestAllowed":totalguest_rows[0]?.totalguest,"totalCount":guestattended_rows[0]?.totalGuest,"acceptCount":accepted_rows[0]?.totalGuest,"declinedCount":rejected_rows[0]?.totalGuest,"nullCount":nullCount,"attendedCount":attended_rows[0]?.totalGuest,notinvitedCount:notinvited_rows[0]?.total});	
...
try {
  ...
  if(id != ''){
    ...
    loadImage(rows[0].entranceURL).then((image) => {
                  w = Math.floor(image.naturalWidth); 
                  h = Math.floor(image.naturalHeight); 
                  const canvas = createCanvas(w, h)
				  console.log(w, h)
                  const ctx = canvas.getContext('2d')               
                  loadImage(rows[0].entranceURL).then((image2) => {
				  var width = Math.floor(image2.naturalWidth); 
                  var height = Math.floor(image2.naturalHeight);                   
                  ctx.drawImage(image2, 0, 0, width, height)
                  loadImage(url).then((img) => {
          let qrw = parseInt((rows[0].QRsize));
          ctx.drawImage(img, w*rows[0].QRW, h*rows[0].QRH, qrw, qrw)
		  console.log(w*rows[0].QRW, h*rows[0].QRH, qrw, qrw)
                          ctx.font = "'"+ rows[0].TextType +"'";  //'48px DTNASKH0'; //"'"+ rows[0].TextType +"'"; //'48px Impact';
                          ctx.fillStyle = rows[0].Textcolor; //"#a46716";//a46716
        ctx.fillText(rows[0].numberMessage+ " " +guests , w*rows[0].NumberW, h*rows[0].NumberH) //w*0.4, h*0.69
        ctx.textAlign = "center";
        ...
        if(rows[0].SNW == null)
          ...
          ctx.fillText(SN, (w*0.5), (h*rows[0].SNH) )     // w*rows[0].SNW
                       ...
                       fs.writeFile("\/home\/tarneem\/Hayyacom\/public\/photo\/iflowerinvitation\/temp\/"+qr_code_file_name, base64Data, 'base64', function(err) {
                               console.log('err',err);
